...
> This is a hard fork of the XeroLinux Toolkit. The original only runs on XeroLinux — it checks `/etc/os-release` and refuses to start on anything else. CyberXero removes that restriction entirely, replaces all distribution-specific metapackages with explicit package lists, rewrites multiple pages from the ground up, adds several features the original never shipped, and migrates deprecated APIs. If it has `pacman`, it runs.
...
### Emulators
Native emulator setup with a unified `~/Emulation/` filesystem. One-click install and configuration for:

- **ES-DE Frontend** — game launcher that ties everything together, pointed at `~/Emulation/roms`
...
Every emulator is automatically configured to use `~/Emulation/` paths for ROMs, BIOS, saves, states, and screenshots. The filesystem setup dialog detects which emulators are already installed and offers to reconfigure them.
